...
const props = defineProps({
  ...
  modelValue: {
    type: [Date, Array, null] as PropType<Date | [Date, Date] | null>,
    required: true,
  },
  picker: {
...
const date = computed(() => {
  ...
  if (props.modelValue instanceof Date) return dayjs(props.modelValue);
  else return [dayjs(props.modelValue[0]), dayjs(props.modelValue[1])];
});
...
const emit = defineEmits<{
  (event: 'update:model-value', value: Date | [Date, Date] | null): void;
}>();
</script>
...
<template>
  ...
  >
    <DatePicker
      ...
      @update:value="
        (v) => {
          ...
          emit('update:model-value', v.toDate());
        }
      "
    >
    ...
    <RangePicker
      v-else
      :value="(date as [string,string]|[Dayjs,Dayjs])"
      :picker="picker"
      :allow-clear="allowClear"
      :bordered="bordered"
      :format="dateFormat"
      ...
      @update:value="
        (v) => {
          if (typeof v[0] === 'string' || typeof v[1] === 'string') return;
          emit('update:model-value', [v[0].toDate(), v[1].toDate()]);
        }
      "
    >
      <template v-if="isDateRender" #dateRender="dateRender">
